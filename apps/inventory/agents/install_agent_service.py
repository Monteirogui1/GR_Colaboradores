...
# ═══════════════════════════════════════════════════════════════
# Widget: Canvas Animado (partículas de fundo no painel esq.)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# Widget: StepBar vertical (painel esquerdo)
# ═══════════════════════════════════════════════════════════════
class StepBar(tk.Frame):
    ...
    def _build(self):
        ...
        logo_c = tk.Canvas(logo_f, width=36, height=36,
        ...
        logo_c.create_text(cx, cy, text="IA", fill=C["white"],
        ...
        for i, (num, title, sub) in enumerate(STEPS):
            ...
            lbl_title = tk.Label(txt_f, text=title,
                                 font=("Segoe UI", 10, "bold"),
                                 bg=C["bg"], fg=C["muted"], anchor="w")
            lbl_title.pack(fill=tk.X)
            lbl_sub = tk.Label(txt_f, text=sub,
                               font=("Segoe UI", 8),
    ...
    def _draw_circle(self, canvas, num, state):
        ...
        if state == "done":
            ...
            canvas.create_text(16, 16, text="✓", fill=C["white"],
                               font=("Segoe UI", 11, "bold"))
        elif state == "active":
            canvas.create_oval(2, 2, 30, 30, fill=C["step_act"], outline=C["accent"], width=2)
            canvas.create_text(16, 16, text=num, fill=C["white"],
                               font=("Consolas", 9, "bold"))
        else:
            canvas.create_oval(2, 2, 30, 30, fill=C["step_idle"],
                               outline=C["border"], width=1)
            canvas.create_text(16, 16, text=num, fill=C["muted"],
                               font=("Consolas", 9))
    ...
    def set_step(self, idx):
        ...
        for i, (canvas, num) in enumerate(self._circles):
            ...
            if i < idx:
                t_lbl.config(fg=C["success"])
                s_lbl.config(fg=C["step_done"])
            elif i == idx:
                t_lbl.config(fg=C["white"])
                s_lbl.config(fg=C["muted"])
            else:
                t_lbl.config(fg=C["muted"])
                s_lbl.config(fg=C["border"])
...
def _label(parent, text, size=9, fg=None, bold=False):
    font_spec = ("Segoe UI", size, "bold") if bold else ("Segoe UI", size)
    return tk.Label(parent, text=text, font=font_spec,
...
class AgentInstaller:
    ...
    def _build(self):
        ...
        # Cabeçalho direito
        self._hdr_f = tk.Frame(right, bg=C["panel"], pady=0)
        self._hdr_f.pack(fill=tk.X)
        self._hdr_title = tk.Label(
            self._hdr_f, text="", font=("Segoe UI", 16, "bold"),
            bg=C["panel"], fg=C["white"], anchor="w", pady=24, padx=28)
        self._hdr_title.pack(side=tk.LEFT)
        self._hdr_sub = tk.Label(
            self._hdr_f, text="", font=("Segoe UI", 9),
            bg=C["panel"], fg=C["muted"], anchor="e", padx=28)
        ...
        # Páginas
        self._pages = []
        self._pages.append(self._page_welcome(self._content))
        self._pages.append(self._page_config(self._content))
        self._pages.append(self._page_install(self._content))
        self._pages.append(self._page_done(self._content))
    ...
    def _mk_btn(self, parent, text, cmd, bg=C["surface"], fg=C["text"],
        ...
        b = tk.Button(parent, text=text, command=cmd,
                      ...
                      font=font_spec, padx=20, pady=8,
                      cursor="hand2", state=state,
                      activebackground=C["accent2"],
                      activeforeground=C["white"])
        b.bind("<Enter>", lambda _: b.config(bg=C["accent2"], fg=C["white"]) if str(b["state"]) != "disabled" else None)
        b.bind("<Leave>", lambda _: b.config(bg=bg, fg=fg) if str(b["state"]) != "disabled" else None)
        return b
    ...
    def _page_welcome(self, parent):
        ...
        tk.Label(arch, text="ARQUITETURA DO SISTEMA",
                 font=("Consolas", 8, "bold"), bg=C["surface"], fg=C["accent"]
        ...
        def arch_block(parent_, title, items, accent_col):
            blk = tk.Frame(parent_, bg=C["panel"],
                           highlightthickness=1,
                           highlightbackground=accent_col)
            blk.pack(fill=tk.X, pady=4)
            hdr = tk.Frame(blk, bg=accent_col)
            hdr.pack(fill=tk.X)
            tk.Label(hdr, text=title, font=("Segoe UI", 9, "bold"),
                     bg=accent_col, fg=C["white"],
                     pady=6, padx=12, anchor="w").pack(fill=tk.X)
            for item in items:
        ...
        arch_block(diagram,
                   "⚙  Serviço Windows  (Session 0 — sem desktop)",
                   ["• Coleta hardware via PowerShell",
                    "• Envia dados ao servidor Django",
                    f"• IPC local  127.0.0.1:{IPC_PORT}"],
                   "#1a3a5c")
    ...
    def _page_config(self, parent):
        ...
        row.pack(fill=tk.X)
        dir_e = _entry(row, self.install_dir)
        dir_e.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=7)
        btn_b = tk.Button(row, text="…", command=self._browse,
                          bg=C["surface"], fg=C["muted"], relief=tk.FLAT,
                          font=("Segoe UI", 10), padx=12, pady=6,
                          cursor="hand2")
        btn_b.pack(side=tk.LEFT, padx=(8, 0))
        ...
        self._eye_btn = tk.Button(
            tok_row, text="👁", command=self._toggle_token,
            bg=C["surface"], fg=C["muted"], relief=tk.FLAT,
            font=("Segoe UI", 10), padx=10, pady=6, cursor="hand2")
        self._eye_btn.pack(side=tk.LEFT, padx=(8, 0))

        tag_f = tk.Frame(body2, bg=C["panel"])
        tag_f.pack(fill=tk.X, pady=(6, 0))
        tk.Label(tag_f, text="🔒  Não salvo em disco — armazenado apenas em memória do serviço",
                 font=("Segoe UI", 8), bg=C["panel"], fg=C["success"]).pack(anchor="w")
    ...
    def _flash_error(self, msg):
        ...
        tk.Button(top, text="OK", command=top.destroy,
                  ...
                  padx=24, pady=6, cursor="hand2").pack(pady=12)
    ...
    def _set_progress(self, pct, title="", sub=""):
        def _upd():
            ...
            if title:
                self._status_title.config(text=title)
            if sub:
    ...
    def _finish(self, install_dir: Path):
        ...
        # Popula cards da página Concluído
        ...
        self._done_sub.config(
            text="O agente está coletando e enviando dados automaticamente.")
